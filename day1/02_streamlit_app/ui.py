...
def display_history_list(history_df):
    ...
    for i, row in paginated_df.iterrows():
        with st.expander(f"{row['timestamp']} - Q: {row['question'][:50] if row['question'] else 'N/A'}..."):
            ...
            # 冗長度、多様性（新規）
            cols = st.columns(2)
            cols[0].metric("冗長度", f"{row['redundancy_score']:.4f}" if pd.notna(row['redundancy_score']) else "-")
            cols[1].metric("多様性", f"{row['diversity_score']:.4f}" if pd.notna(row['diversity_score']) else "-")
...
def display_metrics_analysis(history_df):
    ...
    metric_options = ["bleu_score", "similarity_score", "relevance_score", "word_count", "redundancy_score", "diversity_score"]
    ...
    stats_cols = ['response_time', 'bleu_score', 'similarity_score', 'word_count', 'relevance_score', 'redundancy_score', 'diversity_score']
