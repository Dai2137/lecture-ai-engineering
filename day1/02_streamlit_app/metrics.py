...
def calculate_metrics(answer, correct_answer):
    ...
    if not answer:
        return bleu_score, similarity_score, word_count, relevance_score, redundancy_score, diversity_score

    # 単語分割（日本語でも冗長度・多様性に使えるレベルで分割）
    tokenizer = Tokenizer()
    tokens = [token.surface for token in tokenizer.tokenize(answer)]
    word_count = len(tokens)

    # 多様性（ユニーク語彙の割合）
    if word_count > 0:
        diversity_score = len(set(tokens)) / word_count
    ...
    if word_count > 0:
        ...
        redundancy_score = most_common_count / word_count
    ...
    if correct_answer:
        ...
        try:
            ...
            candidate = nltk_word_tokenize(answer_lower)
            if candidate:
                bleu_score = nltk_sentence_bleu(reference, candidate, weights=(0.25, 0.25, 0.25, 0.25))
        except Exception:
            bleu_score = 0.0

        try:
            vectorizer = TfidfVectorizer()
            if answer_lower.strip() and correct_answer_lower.strip():
                tfidf_matrix = vectorizer.fit_transform([answer_lower, correct_answer_lower])
                similarity_score = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:2])[0][0]
        except Exception:
            similarity_score = 0.0
...
def get_metrics_descriptions():
    ...
    return {
        ...
        "単語数 (word_count)": "回答に含まれる単語の数。情報量や詳細さの指標",
        "関連性スコア (relevance_score)": "正解と回答の共通単語の割合。トピックの関連性を表す (0〜1の値)",
        "冗長度スコア (redundancy_score)": "最頻出単語が回答全体に占める割合。高いほど同じ語が繰り返されている",
        "多様性スコア (diversity_score)": "異なる単語の割合（語彙の多様さ）= ユニーク単語数 ÷ 総単語数",
        "効率性スコア (efficiency_score)": "正確性を応答時間で割った値。高速で正確な回答ほど高スコア"
    }